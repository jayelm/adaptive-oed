var infrastructure = function() {
    ...
    var mSample = function() {
        var aList = fixedStructure;
        if (aList === null) {
            err('fixedStructure is null');
        }
        var aWeights = sampleWeights(aList);
        var aPriors = samplePriors(aList);
        if (cacheEverything) {
            return Model(
                JSON.stringify([aList, aWeights, aPriors]),
                function(x, y) {
                    console.log(x, y);
            ...
            return DAG(aList, aWeights, aPriors);
        }
    };
